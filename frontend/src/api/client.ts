...
export const api = {
  ...
  bulkUpdateUsers: (data: { user_ids: number[]; is_active: boolean }) =>
    apiFetch<{ id: number; name: string; is_active: boolean }[]>("/api/users/bulk", {
      method: "PATCH",
      body: JSON.stringify(data),
    }),

  // Stats
  getUserStats: (userId: number, range: string = "24h") => apiFetch<Stats>(`/api/users/${userId}/stats?range=${range}`),
  getUserSummary: (userId: number) => apiFetch<UserSummary>(`/api/users/${userId}/summary`),
  getFleetStats: (range: string = "24h") => apiFetch<FleetStats>(`/api/stats?range=${range}`),
  ...
  generateConfig: (userId: number, rotate: boolean = false, client: string = "opencode") =>
    apiFetch<GeneratedConfig>(`/api/users/${userId}/config`, {
      method: "POST",
      body: JSON.stringify({ user_id: userId, rotate, client }),
    }),

  // Model CRUD
  listModels: () => apiFetch<Model[]>("/api/models"),
  getModel: (modelId: string) => apiFetch<Model>(`/api/models/${modelId}`),
  createModel: (model: Partial<Model>) =>
    apiFetch<{ model: Model; pending_restart: boolean }>("/api/models", {
      method: "POST",
      body: JSON.stringify(model),
    }),
  updateModel: (modelId: string, model: Partial<Model>) =>
    apiFetch<{ model: Model; pending_restart: boolean }>(`/api/models/${modelId}`, {
      method: "PUT",
      body: JSON.stringify(model),
    }),
  ...
  setModelUsers: (modelId: string, userIds: number[]) =>
    apiFetch<{ updated: boolean }>(`/api/models/${modelId}/users`, {
      method: "PUT",
      body: JSON.stringify({ user_ids: userIds }),
    }),
  exportModels: () => apiFetch<{ json: string }>("/api/models/export", { method: "POST" }),
  importModels: (jsonContent: string) =>
    apiFetch<{ imported: number }>("/api/models/import", {
      method: "POST",
  ...
  setUserDefaultModel: (userId: number, modelId: string | null) =>
    apiFetch<{ updated: boolean }>(`/api/users/${userId}/default-model`, {
      method: "PUT",
      body: JSON.stringify({ model_id: modelId }),
    }),
  getUserDefaultModel: (userId: number) => apiFetch<Model | null>(`/api/users/${userId}/default-model`),
  ...
  revertField: (modelId: string, field: string, value: unknown) =>
    apiFetch<{ model: Model; pending_restart: boolean }>(`/api/models/${modelId}/revert-field`, {
      method: "POST",
  ...
  fetchPost: (path: string, body: Record<string, unknown>) =>
    apiFetch<Record<string, unknown>>(path, { method: "POST", body: JSON.stringify(body) }),
  ...
  testModel: (modelId?: string) =>
    ...
    }>("/api/model/test", { method: "POST", body: JSON.stringify({ model_id: modelId || null }) }),
  ...
  checkModelPath: (path: string) =>
    apiFetch<{ exists: boolean }>(`/api/download/path-exists?path=${encodeURIComponent(path)}`),
  ...
  createModelConfig: (data: { hf_model: HFModel; target_dir: string; gpu_indices: number[] }) =>
    apiFetch<{ model_id: string; config: Record<string, unknown> }>("/api/download/model-config", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  getDownloadStatus: () => apiFetch<{ downloads: DownloadJob[] }>("/api/download/status"),
