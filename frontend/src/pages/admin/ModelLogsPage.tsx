...
function LogLineRow({ line }: { line: LogLine }) {
  ...
  const display = line.timestamp
    ? `${line.timestamp} ${line.content}`
    : line.text

  return (
    <div className={`font-mono text-xs leading-relaxed ${colorClass} whitespace-pre-wrap break-all`}>
      {display}
    </div>
  )
}
          ...
          <div className="flex items-center gap-2 ml-auto">
            <div className="flex items-center gap-1.5 text-xs">
              <span className={`w-2 h-2 rounded-full ${
                status === 'streaming' ? 'bg-emerald-400' :
            ...
            {stats.errorCount > 0 && (
              <span className="text-xs text-red-400">{stats.errorCount} errors</span>
            )}
            {stats.warnCount > 0 && (
              <span className="text-xs text-amber-400">{stats.warnCount} warnings</span>
            )}
          </div>
