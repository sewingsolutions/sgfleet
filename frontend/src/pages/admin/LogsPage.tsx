    ...
    <div>
      <div className={card + " mb-3 sm:mb-6"}>
        ...
        <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-3">
          Controls both stdout output and DB persistence. Current:{" "}
          <code className="text-indigo-600 dark:text-indigo-400">{currentLevel}</code>
        </p>
      ...
      <div className={card + " mb-3 sm:mb-6"}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2">
          ...
          <input
            ...
            className="px-2 py-1.5 bg-gray-100 dark:bg-slate-900 border border-gray-300 dark:border-slate-600 rounded text-xs sm:text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-slate-500"
          />
          <div className="text-xs text-gray-400 dark:text-gray-500 flex items-center">{logs.length} entries</div>
        </div>
      </div>
        ...
        <div className={card + " overflow-x-auto"}>
          <table className="w-full text-xs sm:text-sm">
            ...
            <tbody>
              {logs.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-2 py-6 text-center text-gray-400 dark:text-gray-500">
                    No logs found
                  </td>
                </tr>
              )}
              {logs.map((log, i) => (
                <tr
                  key={log.id}
                  className={`border-t border-gray-200 dark:border-slate-700 ${i % 2 ? "bg-gray-100/50 dark:bg-slate-800/50" : "bg-white dark:bg-slate-800"}`}
                >
                  ...
                  <td className="px-2 py-1.5">
                    <span
                      className={`px-2 py-0.5 rounded text-xs font-medium ${levelColors[log.level as keyof typeof levelColors] || "bg-gray-100 dark:bg-slate-700 text-gray-500"}`}
                    >
                      {log.level}
                    </span>
                  </td>
                  <td className="px-2 py-1.5 text-gray-600 dark:text-gray-300 whitespace-nowrap">{log.method || ""}</td>
                  ...
                  <td className="px-2 py-1.5 text-gray-600 dark:text-gray-300 whitespace-nowrap">{log.user || "-"}</td>
                  <td
                    className="px-2 py-1.5 text-gray-700 dark:text-gray-300 max-w-xs truncate"
