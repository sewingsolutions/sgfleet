...
export default function AuditPage() {
  ...
  if (!logs || logs.length === 0) return <div className="py-8 text-center text-gray-400 dark:text-gray-500">No audit entries yet</div>
        ...
        >
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-2 min-w-0">
              <span className={`px-2 py-0.5 rounded text-xs font-medium shrink-0 ${actionColors[log.action] || 'bg-gray-100 text-gray-700 dark:bg-slate-700 dark:text-gray-300'}`}>
            ...
            <span className="text-gray-400 dark:text-gray-500 text-xs font-mono whitespace-nowrap shrink-0">{log.ip_address || '—'}</span>
          </div>
          ...
          {log.detail && (
            <p className="mt-2 text-xs text-gray-600 dark:text-gray-400 break-all bg-gray-50 dark:bg-slate-900 rounded px-2 py-1 max-w-full overflow-x-auto">{log.detail}</p>
          )}
        </div>
      ))}
    </div>
