...
export default function ModelsPage() {
  ...
  const { mutateAsync: clearPendingRestart } = useMutation({
    mutationFn: (modelId: string) => api.clearPendingRestart(modelId),
    onSuccess: () => refreshInvalidate(),
    onError: (e: Error) => showToast(e.message),
  });
  ...
  const handleDuplicate = (m: Model) => navigate("/admin/models/new", { state: { duplicateFrom: m.model_id } });
            ...
            return (
              ...
              >
                <input
                  ...
                  className="hidden"
                  ref={(el) => {
                    singleFileRefs.current[m.model_id] = el;
                  }}
                ...
                <div className="p-3 sm:p-4">
                      ...
                      {m.pending_restart && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800 shrink-0">
                          ...
                          <button
                            onClick={async () => {
                              try {
                                await clearPendingRestart(m.model_id);
                              } catch (e) {
                                showToast(`Dismiss failed: ${(e as Error).message}`);
                              }
                            }}
                  ...
                  <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500 dark:text-gray-400">
                    ...
                    <span>
                      Path: <code className="font-mono">{m.model_path}</code>
                    </span>
                  </div>
      ...
      {jsonModalId && (
        ...
        >
          ...
          >
            ...
            <pre className="p-4 text-xs font-mono text-gray-800 dark:text-gray-200 overflow-auto flex-1 whitespace-pre-wrap break-all">
              {jsonModalContent}
            </pre>
          </div>
        </div>
