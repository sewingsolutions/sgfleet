...
const UserStatCard = ({ u }: { u: { user: string; p50: number; p95: number; c429: number } }) => (
  <div className={`${card} p-2 sm:p-3`}>
    <div className="flex items-center justify-between mb-1">
      ...
      <span
        className={`px-1.5 py-0.5 rounded text-xs ${u.p95 > 10 ? "bg-red-50 dark:bg-red-900/50 text-red-700 dark:text-red-300" : u.p95 > 1 ? "bg-amber-50 dark:bg-amber-900/50 text-amber-700 dark:text-amber-300" : "bg-emerald-50 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300"}`}
      >
        {u.p95}s
      </span>
    </div>
...
const RequestCard = ({
  ...
  <div className={`${card} p-2 sm:p-3`}>
    <div className="flex items-center justify-between mb-1">
      ...
      <span
        className={`px-1.5 py-0.5 rounded text-xs flex-shrink-0 ml-2 ${
          r.status >= 500
...
export default function MetricsPage() {
  ...
  const activeId = view === "user" ? (selectedId ?? (users.length > 0 ? users[0].id : undefined)) : undefined;
  ...
  const loading =
    loadingUsers || (view === "user" ? statsQuery.isLoading && userRequestsQuery.isLoading : fleetQuery.isLoading);

  const chartOpts = {
    responsive: true,
    maintainAspectRatio: false,
    plugins: {
      legend: { display: true, position: "top" as const, labels: { color: chartTheme.legendColor, boxWidth: 12 } },
    },
    scales: {
  ...
  const quotaPercent =
    summary && summary.daily_quota ? Math.min(100, (summary.today_requests / summary.daily_quota) * 100) : null;

  if (loading) return <div className="py-8 text-center text-gray-500 dark:text-gray-400">Loading...</div>;
  if (view === "user" && !activeId)
    return <div className="py-8 text-center text-gray-500 dark:text-gray-400">No users available</div>;
  if (view === "user" && !stats)
    return <div className="py-8 text-center text-gray-500 dark:text-gray-400">No stats available</div>;
  if (view === "fleet" && !fleet)
    return <div className="py-8 text-center text-gray-500 dark:text-gray-400">No fleet data available</div>;
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500 mb-3">
          Download and import these JSON files into your Grafana instance to monitor SGLang and gateway metrics.
        </p>
        ...
        <div key={`fleet-${range}`}>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
            ...
            <div className={card}>
              ...
              <p
                className={`text-lg sm:text-2xl font-bold truncate ${fleet.avg_latency > 5 ? "text-red-600 dark:text-red-400" : fleet.avg_latency > 1 ? "text-yellow-600 dark:text-yellow-400" : "text-emerald-600 dark:text-emerald-400"}`}
              >
            ...
            <div className={card}>
              ...
              <p
                className={`text-lg sm:text-2xl font-bold truncate ${fleet.total_429 > 0 ? "text-red-600 dark:text-red-400" : "text-emerald-600 dark:text-emerald-400"}`}
              >
                {fmt(fleet.total_429)}
              </p>
            </div>
              ...
              <Line
                ...
                data={{
                  ...
                  datasets: [
                    {
                      label: "429s",
                      data: fleet.count_429,
                      borderColor: chartTheme.c429,
                      backgroundColor: `${chartTheme.c429}20`,
                      tension: 0.3,
                      fill: true,
                    },
                  ],
        ...
        <div key={`${selectedId}-${range}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6">
              ...
              <div className={`${card} sm:col-span-2`}>
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                  P: {fmt(summary.prompt_tokens)} / C: {fmt(summary.completion_tokens)}
                </p>
              </div>
              <div className={`${card} sm:col-span-2`}>
          ...
          <div className="grid grid-cols-1 gap-4 sm:gap-6">
            <CC title="Requests / hour">
              <Line
                options={{ ...chartOpts, plugins: { ...chartOpts.plugins, legend: { display: false } } }}
                data={makeData("Requests", stats.requests, chartTheme.requests)}
              />
            </CC>
            <CC title="429 / hour">
              <Line
                options={{ ...chartOpts, plugins: { ...chartOpts.plugins, legend: { display: false } } }}
                data={makeData("429s", stats.count_429, chartTheme.c429)}
              />
            </CC>
            <CC title="Cost / hour ($)">
              <Line
                options={{ ...chartOpts, plugins: { ...chartOpts.plugins, legend: { display: false } } }}
                data={makeData("Cost", stats.costs, chartTheme.cost)}
              />
            </CC>
              ...
              <Line
                ...
                data={{
                  ...
                  datasets: [
                    {
                      label: "Prompt",
                      data: stats.prompt_tokens,
                      borderColor: chartTheme.prompt,
                      backgroundColor: `${chartTheme.prompt}20`,
                      tension: 0.3,
                      fill: true,
                    },
                    {
                      label: "Completion",
                      data: stats.completion_tokens,
                      borderColor: chartTheme.completion,
                      backgroundColor: `${chartTheme.completion}20`,
                      tension: 0.3,
                      fill: true,
                    },
                  ],
          ...
          {recentRequests && recentRequests.length > 0 && (
            <div className="mt-4 sm:mt-6">
              <h3 className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-2">
                Recent Requests ({recentRequests.length} in {range})
              </h3>
