...
export default function SettingsPage() {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (rateLimitRef.current && rateLimitRef.current.value)
      data.default_rate_limit = parseFloat(rateLimitRef.current.value);
    if (concurrentRef.current && concurrentRef.current.value)
      data.default_max_concurrent = parseInt(concurrentRef.current.value);
  ...
  const handleRotateAdminKey = async () => {
    if (
      !(await confirmAction(
        "Rotate admin key? The CURRENT key will be invalidated immediately. Make sure to copy the new key!",
        true,
      ))
    )
      return;
    ...
    <div>
      <div className="bg-gray-50 dark:bg-slate-800 rounded-lg p-3 sm:p-6 border border-gray-200 dark:border-slate-700 mb-3 sm:mb-6">
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-3 sm:mb-4 text-xs sm:text-sm">
          Default values applied when creating a new user.
        </p>
      ...
      <div className="bg-gray-50 dark:bg-slate-800 rounded-lg p-3 sm:p-6 border border-gray-200 dark:border-slate-700 mb-3 sm:mb-6">
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-3 sm:mb-4 text-xs sm:text-sm">
          API token for downloading gated models. Get one at{" "}
          <a
      ...
      <div className="bg-gray-50 dark:bg-slate-800 rounded-lg p-3 sm:p-6 border border-gray-200 dark:border-slate-700 mb-3 sm:mb-6">
        ...
        {health && (
          <div className="mt-4 pt-4 border-t border-gray-200 dark:border-slate-700 space-y-2 text-xs sm:text-sm">
            ...
            <div>
              ...
              <span className="ml-2">
                {health.model_loaded ? (
                  ...
                  <span className="text-amber-600 dark:text-amber-400">not loaded</span>
                )}
              </span>
            </div>
            {health.container && (
              <>
                <div>
                  ...
                  <code className="ml-2 text-gray-700 dark:text-gray-300">{health.container.name}</code>
                </div>
                <div>
                  <span className="text-gray-500 dark:text-gray-400">State:</span>{" "}
                  <span
                    className={`ml-2 ${health.container.state === "running" ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}
                  >
                ...
                <div>
                  ...
                  <span className="ml-2 text-gray-700 dark:text-gray-300">{health.container.restart_count}</span>
                </div>
                {health.container.health_status && health.container.health_status !== "none" && (
                  <div>
                    <span className="text-gray-500 dark:text-gray-400">Docker Health:</span>{" "}
                    <span
                      className={`ml-2 ${health.container.health_status === "healthy" ? "text-emerald-600 dark:text-emerald-400" : "text-amber-600 dark:text-amber-400"}`}
                    >
            ...
            <div>
              <span className="text-gray-500 dark:text-gray-400">Admin uptime:</span>{" "}
              <span className="ml-2 text-gray-700 dark:text-gray-300">{formatUptime(health.admin.uptime_seconds)}</span>
            </div>
            <div>
              <span className="text-gray-500 dark:text-gray-400">Admin memory:</span>{" "}
              <span className="ml-2 text-gray-700 dark:text-gray-300">{health.admin.memory_mb} MB</span>
            </div>
            <div>
              <span className="text-gray-500 dark:text-gray-400">Last check:</span>{" "}
              <span className="ml-2 text-gray-400 dark:text-gray-500">{health.last_checked}</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="bg-gray-50 dark:bg-slate-800 rounded-lg p-3 sm:p-6 border border-gray-200 dark:border-slate-700 mb-3 sm:mb-6">
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-3 sm:mb-4 text-xs sm:text-sm break-all">
          Paste JSON array of user objects:{" "}
          <code className="bg-gray-100 dark:bg-slate-900 px-1 rounded">
            {'[{"name": "user1", "rate_limit": 2, "daily_quota": 1000}]'}
          </code>
        </p>
        <textarea
          value={importJson}
        ...
        {importResult && (
          <div className="mt-3 text-sm">
            {importResult.created.length > 0 && (
              <p className="text-emerald-600 dark:text-emerald-400">Created: {importResult.created.join(", ")}</p>
            )}
            {importResult.skipped.length > 0 && (
              <p className="text-gray-400 dark:text-gray-500">Skipped (exists): {importResult.skipped.join(", ")}</p>
            )}
          </div>
        )}
      </div>
      ...
      <div className="bg-gray-50 dark:bg-slate-800 rounded-lg p-3 sm:p-6 border border-gray-200 dark:border-slate-700 mb-3 sm:mb-6">
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-3 sm:mb-4 text-xs sm:text-sm">
          Database at <code className="bg-gray-100 dark:bg-slate-900 px-1 rounded">/data/admin.db</code>
        </p>
        <div className="flex flex-wrap gap-2 mb-4">
          <button
            onClick={handleExportUsers}
            className="px-4 py-2 bg-gray-200 dark:bg-slate-700 hover:bg-gray-300 dark:hover:bg-slate-600 rounded text-gray-900 dark:text-white text-sm transition"
          >
        ...
        {exportJson && (
          <div className="mt-3">
            <pre className="bg-gray-100 dark:bg-slate-900 border border-gray-300 dark:border-slate-600 rounded p-3 text-xs text-gray-700 dark:text-gray-300 overflow-auto max-h-64">
              {exportJson}
            </pre>
          </div>
        )}
      ...
      <div className="bg-gray-50 dark:bg-slate-800 rounded-lg p-3 sm:p-6 border border-gray-200 dark:border-slate-700 mb-3 sm:mb-6">
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-3 sm:mb-4 text-xs sm:text-sm">
          Generate a new admin API key. The old key will be invalidated immediately.
        </p>
        <button
          onClick={handleRotateAdminKey}
          className="px-4 py-2 bg-amber-600 hover:bg-amber-700 rounded text-gray-900 dark:text-white text-sm transition"
        >
          Rotate Admin Key
        </button>
        {newAdminKey && (
      ...
      <div className="bg-gray-50 dark:bg-slate-800 rounded-lg p-3 sm:p-6 border border-gray-200 dark:border-slate-700 mb-3 sm:mb-6">
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-3 sm:mb-4 text-xs sm:text-sm break-all">
          Send notifications to external URLs on events: quota_warning, quota_exceeded, key_rotated, user_disabled,
          rate_limited_spike
        </p>
        <div className="space-y-3">
          {webhooks.map((w) => (
            <div
              key={w.id}
              className="bg-gray-100 dark:bg-slate-900 border border-gray-300 dark:border-slate-600 rounded p-3 flex items-start justify-between"
            >
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <code className="text-gray-900 dark:text-white font-medium">{w.name}</code>
                  <span
                    className={`px-2 py-0.5 rounded text-xs font-medium ${w.is_active ? "bg-emerald-50 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300" : "bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400"}`}
                  >
        ...
        <div className="mt-4 space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {events.map((ev) => (
              <label
                key={ev}
                className={`px-3 py-1 rounded-full text-xs cursor-pointer transition ${newWebhookEvents.includes(ev) ? "bg-indigo-600 text-gray-900 dark:text-white" : "bg-gray-200 dark:bg-slate-700 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white"}`}
              >
                <input
                  type="checkbox"
                  ...
                  onChange={(e) => {
                    setNewWebhookEvents(
                      e.target.checked ? [...newWebhookEvents, ev] : newWebhookEvents.filter((x) => x !== ev),
                    );
                  }}
                  className="hidden"
                />
      ...
      <div className="bg-gray-50 dark:bg-slate-800 rounded-lg p-3 sm:p-6 border border-gray-200 dark:border-slate-700">
        ...
        <button
          onClick={async () => {
            if (await confirmAction("Log out?", true)) logout();
          }}
...
function BaseUrlSection() {
  ...
  return (
    <div className="bg-gray-50 dark:bg-slate-800 rounded-lg p-3 sm:p-6 border border-gray-200 dark:border-slate-700 mb-3 sm:mb-6">
      ...
      <p className="text-gray-500 dark:text-gray-400 mb-3 sm:mb-4 text-xs sm:text-sm">
        External URL used in generated API configs (e.g. opencode.json). Clients will be configured to point to this
        address.
      </p>
      {isLoading ? (
        <p className="text-gray-500 dark:text-gray-400 text-sm">Loading...</p>
