...
export default function UserDashboardPage() {
  ...
  useEffect(() => {
    ; (async () => {
      try {
        const [quota, modelsRes] = await Promise.allSettled([
          api.user.getQuota(),
          api.user.getModels(),
        ])
        const todayRequests = quota.status === 'fulfilled' ? quota.value?.today_requests ?? 0 : 0
        const totalRequests = quota.status === 'fulfilled' ? quota.value?.total_requests ?? 0 : 0
        const totalTokens = quota.status === 'fulfilled' ? quota.value?.total_tokens ?? 0 : 0
        const dailyQuota = quota.status === 'fulfilled' ? quota.value?.daily_quota ?? null : null
        const modelsCount = modelsRes.status === 'fulfilled' ? modelsRes.value?.models?.length ?? 0 : 0
  ...
  const cards = [
    { label: 'Today\'s Requests', value: loading ? '...' : formatNum(stats?.todayRequests ?? 0), color: 'indigo' },
    { label: 'Total Requests', value: loading ? '...' : formatNum(stats?.totalRequests ?? 0), color: 'emerald' },
    { label: 'Total Tokens', value: loading ? '...' : formatNum(stats?.totalTokens ?? 0), color: 'amber' },
    { label: 'Authorized Models', value: loading ? '...' : stats?.models ?? 0, color: 'purple' },
  ]
            ...
            <div className="flex-1">
              <div className="h-4 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (stats.todayRequests / stats.quota) * 100)}%`,
                  }}
                />
              </div>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                {stats.todayRequests} / {formatNum(stats.quota)} requests today
              </p>
            </div>
          </div>
        </div>
