...
export default function UserQuotaPage() {
  ...
  useEffect(() => {
    ; (async () => {
      try {
        const res = await api.user.getQuota()
        setQuota(res)
                ...
                <div className="h-4 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all ${
                      (quota.usage_percent ?? 0) > 90 ? 'bg-red-500' : (quota.usage_percent ?? 0) > 70 ? 'bg-amber-500' : 'bg-indigo-500'
                    }`}
          ...
          <div className="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-xl font-semibold mt-1 text-emerald-600 dark:text-emerald-400">{formatNum(quota.total_prompt_tokens)}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-400">Completion Tokens</p>
                <p className="text-xl font-semibold mt-1 text-amber-600 dark:text-amber-400">{formatNum(quota.total_completion_tokens)}</p>
              </div>
            </div>
          </div>
