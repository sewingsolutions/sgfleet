...
describe("useUserStats", () => {
  ...
  it("query is disabled when userId is undefined", () => {
    renderHook(() => useUserStats(undefined, "24h"), { wrapper: makeWrapper(queryClient) });

    expect(api.getUserStats).not.toHaveBeenCalled();
  });

  it("query is enabled when userId is provided", () => {
    renderHook(() => useUserStats(1, "24h"), { wrapper: makeWrapper(queryClient) });
...
describe("useUserSummary", () => {
  ...
  it("query is disabled when userId is undefined", () => {
    renderHook(() => useUserSummary(undefined), { wrapper: makeWrapper(queryClient) });

    expect(api.getUserSummary).not.toHaveBeenCalled();
  });

  it("query is enabled when userId is provided", () => {
    renderHook(() => useUserSummary(1), { wrapper: makeWrapper(queryClient) });
