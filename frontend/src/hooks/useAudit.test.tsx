...
describe('useAuditLog', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(api.getAuditLog).mockResolvedValue([
      { id: 1, timestamp: '2024-01-01T00:00:00Z', action: 'login', target_user_id: null, detail: 'admin login', ip_address: '127.0.0.1' },
    ])
  })
...
describe('useUserRequests', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(api.getUserRequests).mockResolvedValue([
      { id: 1, timestamp: '2024-01-01T00:00:00Z', user_id: 1, request_id: 'req-1', method: 'POST', endpoint: '/v1/chat', status: 200, latency_ms: 50, error_msg: '' },
    ])
  })

  it('query is disabled when userId is undefined', () => {
    renderHook(
      () => useUserRequests(undefined),
      { wrapper: makeWrapper(queryClient) }
    )
