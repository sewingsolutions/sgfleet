  ...
  useEffect(() => {
    ...
    ; (async () => {
      try {
        ...
        if (!cancelled) setModels(list.filter(m => m.active))
      } catch { /* ignore */ }
    })()
    return () => { cancelled = true }
  }, [])

  const healthPollInterval = 10000
  useEffect(() => {
    let cancelled = false
    const poll = async () => {
      try {
        ...
        const activeIds = active.map(m => m.model_id)
        const results: Record<string, ModelHealth> = {}
        await Promise.allSettled(
          activeIds.map(async (id) => {
            try {
              results[id] = await api.getModelHealth(id)
            } catch { /* ignore */ }
          })
        )
        if (!cancelled) {
    ...
    <div className="min-h-screen bg-white dark:bg-slate-900 text-gray-900 dark:text-gray-100 flex flex-col">
      ...
      <footer className="border-t border-gray-200 dark:border-slate-800 py-3 px-4 sm:px-6">
        <div className="max-w-[1600px] mx-auto flex justify-center">
          <Link to="/version" className="text-xs text-gray-400 dark:text-gray-600 hover:text-indigo-600 dark:hover:text-indigo-400 transition font-mono">
