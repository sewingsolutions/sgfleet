...
export function useCreateUserMutation() {
  ...
  return useMutation({
    mutationFn: (data: { name: string; rate_limit?: number; max_concurrent?: number; request_cost?: number; daily_quota?: number | null }) =>
      api.createUser(data),
...
export function useBulkUpdateMutation() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (variables: { user_ids: number[]; is_active: boolean }) =>
      api.bulkUpdateUsers(variables),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['users'] }),
  })
}
